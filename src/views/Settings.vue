  ...
  <div class="min-h-screen bg-gray-900 text-white p-4">
    <div class="max-w-4xl mx-auto">
      ...
      <div class="bg-gray-800 rounded-lg p-6 mb-6">
        ...
        <div v-for="service in filteredServices" :key="service.id" class="mb-8">
          <div class="flex justify-between items-center mb-3">
            <h3 class="text-lg font-medium flex items-center">
              <img :src="service.icon" class="w-6 h-6 rounded mr-2" :alt="`${service.name} icon`" />
              {{ service.name }}
            </h3>
            <button
          ...
          >
            <div v-if="getInstancesForService(service.id).length" class="mb-4 space-y-2">
              <div
              ...
              >
                ...
                <div v-if="instance.isDefault" class="absolute top-2 right-2 bg-blue-500 text-white text-xs px-2 py-0.5 rounded-full">
                  Default
                </div>
              </div>
            </div>
            ...
            <button
              ...
              :disabled="!subscriptionStore.hasCustomInstances"
              ...
              :title="!subscriptionStore.hasCustomInstances ? 'Subscription required to add custom instances' : `Add ${service.name} instance`"
            >
        ...
        <div v-if="!filteredServices.length" class="text-center py-6 text-gray-400">
      ...
      <div class="bg-gray-800 rounded-lg p-6 mb-6">
        ...
        <div class="mb-4">
          <div class="flex items-center mb-2">
            <div
              ...
              :class="subscriptionStore.isSubscribed ? 'bg-green-500' : 'bg-red-500'"
            ></div>
            <span class="text-lg font-medium">
              {{ subscriptionStore.isSubscribed ? 'Active Subscription' : 'No Active Subscription' }}
            </span>
          </div>
          <p v-if="subscriptionStore.isSubscribed" class="text-gray-400">
        ...
        <!-- Error Message -->
        <div v-if="subscriptionStore.error" class="mb-4 p-3 bg-red-900/50 border border-red-700 rounded-lg">
          <p class="text-red-400 text-sm">{{ subscriptionStore.error }}</p>
        </div>
    ...
    >
      <div class="bg-gray-800 rounded-lg w-full max-w-md p-6" role="document">
        <div class="flex items-center mb-4">
          <img v-if="currentService" :src="currentService.icon" class="w-8 h-8 rounded mr-3" :alt="`${currentService.name} icon`" />
          <h3 :id="`${editingInstance ? 'edit' : 'add'}-instance-title`" class="text-xl font-semibold">
    ...
    >
      <div class="bg-gray-800 rounded-lg w-full max-w-md p-6" role="document">
        <h3 id="delete-instance-title" class="text-xl font-semibold mb-4">Delete Instance</h3>
    ...
    >
      <div class="bg-gray-800 rounded-lg w-full max-w-md p-6" role="document">
        <h3 id="default-instance-title" class="text-xl font-semibold mb-4">Change Default Instance</h3>
        <p class="text-gray-300 mb-6">
          Are you sure you want to set "{{ instanceForm.name }}" as the default instance?
          This will replace the current default instance.
        </p>
        <div class="flex justify-end space-x-3">
          <button
...
const saveUserInstances = () => {
  try {
    localStorage.setItem(USER_CUSTOM_INSTANCES_KEY, JSON.stringify(userCustomInstances.value))
  } catch (error) {
...
function validateUrl() {
  ...
  if (currentService.value?.id === 'kodi') {
    ...
    if (!kodiPattern.test(instanceForm.value.baseUrl)) {
      urlError.value = 'Kodi URL should be in format: IP:port (e.g., 192.168.1.100:8080)'
    }
  } else {
...
function getUrlHelpText(serviceId?: string) {
  ...
  if (serviceId === 'kodi') return 'Enter the IP and port of your Kodi instance'
...
async function saveInstance() {
  ...
  try {
    ...
    if (editingInstance.value) {
      ...
      const index = userCustomInstances.value[currentService.value.id].findIndex(i => i.id === editingInstance.value?.id)
      if (index !== -1) {
        userCustomInstances.value[currentService.value.id][index] = instanceData
      }
    } else {
      // Add new instance
    ...
    if (editingInstance.value) {
      ...
      await servicesStore.addCustomInstance(currentService.value.id, instanceData)
    }
...
async function deleteInstance() {
  ...
  try {
    ...
    if (userCustomInstances.value[instanceToDelete.value.serviceId]) {
      userCustomInstances.value[instanceToDelete.value.serviceId] = userCustomInstances.value[instanceToDelete.value.serviceId].filter(i => i.id !== instanceToDelete.value?.instance.id)
    }
...
async function handlePurchase() {
  await subscriptionStore.purchaseSubscription()
}
